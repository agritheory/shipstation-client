...
def mocked_api():
	with mock(base_url="https://ssapi.shipstation.com", assert_all_called=False) as respx_mock:
		...
		respx_mock.get("/orders/123456789", name="get_order").respond(200, json=GET_ORDER)
		respx_mock.get("/products/123456789", name="get_product").respond(200, json=GET_PRODUCT)
		respx_mock.post("/shipments/getrates", name="get_rates").respond(200, json=GET_RATES)
		...
		respx_mock.get("/carriers", name="list_carriers").respond(200, json=LIST_CARRIERS)

		respx_mock.get("/customers", name="list_customers").respond(200, json=LIST_CUSTOMERS)
		...
		respx_mock.get("/shipments", name="list_shipments",).mock(
			side_effect=[
		...
		respx_mock.get("/accounts/listtags", name="list_tags").respond(200, json=LIST_TAGS)
		respx_mock.get("/users", name="list_users").respond(200, json=LIST_USERS)
		respx_mock.get("/warehouses", name="list_warehouses").respond(200, json=LIST_WAREHOUSES)
