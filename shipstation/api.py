...
class ShipStation(ShipStationHTTP):
	...
	def get_carrier(self, carrier_code: str) -> str | ShipStationCarrier:
		...
		carrier = self.get(endpoint="/carriers/getcarrier", payload={"carrierCode": carrier_code})
	...
	def list_carriers(self) -> list[str | ShipStationCarrier]:
		...
		return [
			ShipStationCarrier().json(carrier) for carrier in carriers.json(parse_float=Decimal) if carrier
		]
	...
	def list_packages(self, carrier_code: str) -> list[str | ShipStationCarrierPackage]:
		...
		packages = self.get(endpoint="/carriers/listpackages", payload={"carrierCode": carrier_code})
		return [
			ShipStationCarrierPackage().json(package) for package in packages.json(parse_float=Decimal)
		]
	...
	def list_services(self, carrier_code: str) -> list[str | ShipStationCarrierService]:
		...
		services = self.get(endpoint="/carriers/listservices", payload={"carrierCode": carrier_code})
		return [
			ShipStationCarrierService().json(service) for service in services.json(parse_float=Decimal)
		]
	...
	def list_customers(self, parameters: dict[str, Any] | None = None) -> Page:
		"""
	...
	# FULFILLMENTS
	def list_fulfillments(self, parameters: dict[str, Any] | None = None) -> Page:
		"""
	...
	def create_orders(self, orders: list[ShipStationOrder]) -> list[str | ShipStationOrder]:
		"""
	...
	def hold_order_until(
		...
		data = json.dumps({"orderId": str(order_id), "holdUntilDate": str(hold_until_date)})
	...
	def list_products(self, parameters: dict[str, Any] | None = None) -> Page:
		"""
	...
	def get_rates(self, options: ShipStationRateOptions) -> list[str | ShipStationRate]:
		...
		return [ShipStationRate().json(rate) for rate in rates.json(parse_float=Decimal)]

	def list_shipments(self, parameters: dict[str, Any] | None = None) -> Page:
		"""
	...
	def list_marketplaces(self) -> list[str | ShipStationMarketplace]:
		...
		return [ShipStationMarketplace().json(m) for m in marketplaces.json(parse_float=Decimal) if m]
	...
	def list_users(self, show_inactive: bool = False) -> list[str | ShipStationUser]:
		...
		self.require_type(show_inactive, bool)
		users = self.get(endpoint="/users", payload=json.dumps({"showInactive": show_inactive}))
		return [ShipStationUser().json(user) for user in users.json(parse_float=Decimal) if user]

	# WAREHOUSES
	def create_warehouse(self, data: ShipStationWarehouse) -> str | ShipStationWarehouse:
		"""
	...
	def list_warehouses(self) -> list[str | ShipStationWarehouse]:
		...
		return [ShipStationWarehouse().json(wh) for wh in warehouses.json(parse_float=Decimal) if wh]
